...
public class SmsMessaging extends BaseMessaging {
	...
	public boolean sendImage(byte[] imageData) {
		...
		try {
			...
			int i = 1;
					//smsconn.numberOfSegments(binmsg);
			...
			Exception e = new Exception("Send caught: " + t.getMessage() );
			handler.handle( e );
	...
	public byte[] receiveImage() throws InterruptedIOException, IOException {
		...
		if (smsConn == null) {
			try {
				...
				Exception e = new Exception("Receive caught: " + ioe.getMessage() );
				handler.handle( e );
	...
	public void cleanUpConnections(MessageConnection smsConn) {
		...
		if (smsConn != null) {
			try {
				...
				Exception e = new Exception("Closing connection caught: " + ioe.getMessage() );
				handler.handle( e );
			}
		}
